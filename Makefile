...
build: ## Build the binary
	go build -ldflags "-X main.Version=1.5.0" -o amos

build-windows: ## Build Windows binary (amd64)
	GOOS=windows GOARCH=amd64 go build -ldflags "-X main.Version=1.5.0" -o amos.exe

build-all: ## Build binaries for all platforms
	@echo "Building for Linux (amd64)..."
	GOOS=linux GOARCH=amd64 go build -ldflags "-X main.Version=1.5.0" -o amos-linux-amd64
	@echo "Building for macOS (amd64)..."
	GOOS=darwin GOARCH=amd64 go build -ldflags "-X main.Version=1.5.0" -o amos-darwin-amd64
	@echo "Building for macOS (arm64)..."
	GOOS=darwin GOARCH=arm64 go build -ldflags "-X main.Version=1.5.0" -o amos-darwin-arm64
	@echo "Building for Windows (amd64)..."
	GOOS=windows GOARCH=amd64 go build -ldflags "-X main.Version=1.5.0" -o amos-windows-amd64.exe
...
release: ## Create a new release (usage: make release VERSION=1.2.1 [NOTES=release-notes.md] [DRY_RUN=true])
	@if [ -z "$(VERSION)" ]; then \
		...
		echo "Usage: make release VERSION=1.5.0 [NOTES=release-notes.md] [DRY_RUN=true]"; \
		exit 1; \
	fi
